...
function LandInfoInit(){
	...
	LandInfos.push(land);
	var land = new LandInfo(9,'부에노스아이레스',22,45,75,80,9,70,105,125,0,22,-1,0,"0000",'i');
	LandInfos.push(land);
			...
			for (idx in users) {
				...
				if(users[idx].bankrupt==1 && users[idx].bankrupt_check==0) 
				{
					for (land in users[idx].MyLandInfos)
					{
						...
						LandInfos[land_idx].possibleBuilding = 0;
...
//	var d2 = 6;
	...
	{
		if(users[idx].driftdays == 1)									// 3일이 되었나?
		{
...
server = http.createServer(function(req, res){
  ...
  switch (path){
	...
	case '/themes/jquery.bpopup.min.js':
	case '/themes/bhs_theme_1_3_2_02.css':
	case '/json.js':
    case '/arduino3.html':
      fs.readFile(__dirname + path, function(err, data){
        if (err) return send404(res);
  ...
  client.on('select_result', function(data){
	...
	saveObject=object;
		...
		if( object.status==444 )
		{
			...
			users[turn%users.length].dice_processing=0; ///// ★★★★★★★★★★
			please_wait();		
			console.log('  object.status ==  444 Not Move Arduino ' );
		}
		//        벌금꺼               무인도              복지기금접수         복지기금회수
		else if( object.status==7 ||object.status==8 || object.status== 10 || object.status==11)
		{
	...
	user_idx=getMatchId(client.id)
			...
			if(typeof user_idx != "undefined"){ // Bot 이나 다른 그외의 DisConnect는 허락 안함.
				...
				else	
				{
					...
					// 주사위 프로세싱 중이 아니였다면, 강제로 Please_Wait 발생
					for (idx in users) {
						if(users[idx].isBot==0 && users[idx].bankrupt==0){
							count++;
